...
import { HABITAT_POOL, ANIMAL_POOL } from "@/lib/kids-counting/data/pools"
import type { HabitatType } from "@/lib/kids-counting/data/pools"
import { DEFAULT_CONFIG, type GameConfig } from "@/lib/kids-counting/data/config"
import { Button } from "@/components/ui/button"
import { Slider } from "@/components/ui/slider"

const STORAGE_KEY = "animal-counting-teacher-config"
...
export function AnimalTeacherPanel({ initialConfig, onStartGame, onBack }: TeacherPanelProps) {
    ...
    <main className="min-h-screen bg-gradient-to-b from-blue-50 via-purple-50 to-pink-50 p-6 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <Button variant="outline" className="rounded-xl bg-transparent" onClick={onBack}>
            ← Trò Chơi
          </Button>
        </div>
              ...
              <div className="bg-white rounded-2xl shadow-md p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleSelectAllAnimals} size="sm" variant="outline" className="text-xs bg-transparent">
                      Tất Cả
                    </Button>
                    <Button
            ...
            <div className="bg-white rounded-2xl shadow-md p-6">
              ...
              <div className="flex gap-2 flex-wrap">
                {["easy", "normal", "hard"].map((level) => (
                  <button
                    key={level}
                    onClick={() => setConfig((prev) => ({ ...prev, levelDifficulty: level as GameConfig["levelDifficulty"] }))}
                    className={`px-6 py-3 rounded-lg font-semibold transition-all border-2 ${
